...
for i in 1:len_x
    ...
    append!(γ, count(curr) > len_y - count(curr));     
end

ϵ = .!γ
answer1 = parse(Int, join(map(x -> string(convert(Int64, x)), γ)), base= 2) * parse(Int, join(map(x -> string(convert(Int64, x)), ϵ)), base= 2)
...
function get_residual(nums, oxygen = true)
 ...
 answer2 = get_residual(numbers) * get_residual(numbers, false)
